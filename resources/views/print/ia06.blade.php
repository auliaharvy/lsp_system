  ...
  <div class="row">
    <div class="col-12">
      ...
      <table class="col-md-12 table-bordered table-striped">
        ...
        @php
        $detailIa06 = collect( $data['detail'] );
        @endphp
        @foreach ($detailIa06 as $key => $item)
        <tr>
          <td class="text-center px-2">{{ $key+1 }}</td>
          <td class="justify-content-center px-2">{{ $detailIa06[$key]['kode_unit'] }} <br>
            {{ $detailIa06[$key]['unit_kompetensi'] }}</td>
          <td class="justify-content-center px-2">{{ $detailIa06[$key]['pertanyaan'] }}</td>
          <td class="justify-content-center px-2">{{ $detailIa06[$key]['jawaban'] }}</td>
        </tr>
        @endforeach
      </table>
    </div>
